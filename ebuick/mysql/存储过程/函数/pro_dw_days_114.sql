...
SET v_begin_date = CONCAT(DATE_ADD(p_i_date,INTERVAL -(select WEEKDAY(p_i_date)) DAY),' 00:00:00');-- 获取当前传入日期的周一至现在  如果当前日期是周二  那么这个 v_begin_date=周一那天的日期
SET v_end_date = CONCAT(p_i_date,' 23:59:59');
-- -----------------------------------------------------------------------------------------------------------------------------------------------------------------------
#       重复执行时删除当天数据
DELETE FROM T_SC_VEHICLESERIES_LEADS WHERE THEDATE = DATE_ADD(p_i_date,INTERVAL -(select WEEKDAY(p_i_date)) DAY) and REPORTTYPE =1 ;    
-- -----------------------------------------------------------------------------------------------------------------------------------------------------------------------
...
CDATE,UDATE)
	VALUES (0, DATE_ADD(p_i_date,INTERVAL -(select WEEKDAY(p_i_date)) DAY), v_vehicleseries_id,v_vehicleseries_name,v_vehicleseries_count,v_vehicleseries_num,v_allvehicleseries_count,1,CURDATE(),CURDATE()
);
SET v_fetch_ok = 0;
END IF;
END LOOP;
